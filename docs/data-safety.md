...
## Protected Codex sources

The following paths are strict observation-only sources:

- `%USERPROFILE%\.codex\sessions`
- `%USERPROFILE%\.codex\archived_sessions`
- `%USERPROFILE%\.codex\agents`

The application must never open any file below these paths for writing, lock it, rename it, delete it, truncate it, repair it or create an output beneath it. This rule applies to rollout JSONL files and `agents\*.toml` alike.
...
- `collector-worker.ts` reads rollout files with `stat`, `readFile` and `open(filePath, "r")`. It reads `agents` using `readdir` only. `chokidar` watches the source directories but does not change their contents.
