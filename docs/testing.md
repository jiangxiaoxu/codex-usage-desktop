...
| Responsive UI | 检查 900x720 minimum、1000 DIP 阈值、short height 和 high DPI;点击费用条,再用 `Space`/`Enter` 和 `Escape` | 筛选区保持时间范围、model、执行主体和主线程四个独立行;窄窗口仅标签列自适应且 chips 换行;总体费用构成独占一行并常驻显示总费用金额和四色占比;模型与执行主体卡仅显示对当前筛选总费用的占比,不显示美元金额;点击费用条或按 `Space`/`Enter` 切换稳定内嵌详情,四个类别各占两行,`Escape` 收起,正常刷新不自动关闭;详情默认收起且只在展开时占用高度;卡片在 1000 DIP 切换并排和上下堆叠;页面单一纵向滚动且无 clipping 或横向明细表 |
