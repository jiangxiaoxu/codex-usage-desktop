...
| Responsive UI | 检查 900x720 minimum、1000 DIP 阈值、short height 和 high DPI | 筛选区保持时间范围、model、执行主体和主线程四个独立行;窄窗口仅标签列自适应且 chips 换行;总体费用构成独占一行并常驻显示四色占比;模型与执行主体卡在 1000 DIP 切换并排和上下堆叠;整条费用构成 hover/focus 时显示四色占比;页面单一纵向滚动且无 clipping 或横向明细表 |
