...
The result is `release\winui-installer\codex-usage-desktop-setup-0.3.1-x64.exe`. It is an all-users installer under `%ProgramFiles%` and therefore requires UAC. It can replace the legacy Electron 0.2.6 installation in place while retaining the LocalAppData ledger and migrating the startup choice to HKCU Run. Uninstall removes the installed payload and startup registration but does not remove the ledger by default.
